...
// PhoneMockup과 같은 비율(가로:세로)이 되도록 기준 폭 200px에서 세로를 맞췄다.
const BASE_W = 200;
    ...
    <div className={`h-[406px] w-[200px] lg:h-[386px] lg:w-[190px] ${className}`}>
      <div className="origin-top-left lg:scale-[0.95]" style={{ width: BASE_W }}>
        <div className="animate-float relative rotate-[-5deg]" style={{ animationDelay: "0.4s" }}>
          <div
            className="relative overflow-hidden rounded-[30px] bg-[var(--bg)] pb-4"
            style={{
              width: "100%",
          ...
          >
            ...
            {/* 상태바 */}
            <div className="flex items-center justify-between px-4 pb-1 pt-3.5 text-[8.5px] font-bold text-[var(--text)]">
              <span>9:41</span>
              <span>●●●</span>
            </div>
            ...
            <div className="mt-1 px-4">
              <p className="text-[11.5px] font-extrabold text-[var(--text)]">캘린더</p>
              <p className="mt-0.5 text-[8.5px] font-semibold text-[var(--text-soft)]">한 달의 컨디션 흐름</p>
            </div>

            {/* 월 네비게이션 */}
            <div className="mx-3 mt-2.5 flex items-center justify-between">
              <span className="text-[9.5px] text-[var(--text-soft)]">‹</span>
              <span className="text-[10.5px] font-extrabold text-[var(--text)]">2026년 7월</span>
              <span className="text-[9.5px] text-[var(--text-soft)]">›</span>
            </div>

            {/* 캘린더 그리드 */}
            <div
              className="mx-3 mt-2 rounded-[16px] p-2"
              style={{ background: "var(--card)", boxShadow: "var(--shadow-soft)" }}
            >
              <div className="grid grid-cols-7 gap-[3px]">
                {WEEKDAYS.map((w) => (
                  <span key={w} className="text-center text-[6.5px] font-bold text-[var(--text-soft)]">
                    {w}
                  </span>
                ))}
                {CELLS.map((c, i) => {
                  ...
                  return (
                    <span
                      key={i}
                      className="flex aspect-square items-center justify-center rounded-[5px] text-[7px] font-extrabold"
                      style={{
            ...
            <div
              className="mx-3 mt-2 rounded-[16px] p-3"
              style={{ background: "var(--card)", boxShadow: "var(--shadow-soft)" }}
            >
              <div className="flex items-center justify-between">
                <span className="text-[9.5px] font-extrabold text-[var(--text)]">7월 24일 (목)</span>
                <span
                  className="rounded-full px-2 py-[2px] text-[7.5px] font-bold text-white"
              ...
              <p className="mt-1.5 flex items-baseline gap-1">
                <span className="text-[20px] font-extrabold leading-none text-[var(--text)]">92</span>
                <span className="text-[9px] font-semibold text-[var(--text-soft)]">점</span>
              </p>
              <p className="mt-1.5 text-[8px] leading-snug text-[var(--text-soft)]">
                수면 7.5h · 운동 완료 · 카페인 절제 — 이번 달 최고 기록이에요
              </p>
            </div>

            <div
              className="mx-3 mt-2 rounded-[12px] px-2.5 py-2 text-center text-[8.5px] font-extrabold"
